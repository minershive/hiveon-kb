...
Для T17, можете попробовать нашу тестовую "beta" [прошивку Hiveon](http://download.hiveos.farm/asic/t17/Antminer-T17-Hiveon-1.00-beta-nosign.tar.gz).

http://download.hiveos.farm/asic/t17/Antminer-T17-Hiveon-1.00-beta-nosign.tar.gz
...
- (ERROR_SOC_INIT) - ошибка инициализации драйвера
- (ERROR_REOPEN_CORE) - ошибка при переоткрытии ядер
- (ERROR_FAN_LOST) - потеря одного и более кулеров
- (ERROR_POWER_LOST) - ошибка выставления вольтажа, ошибка определения типа БП
- (ERROR_EEPROM_INFO) - ошибка чтения содержимого EEPROM (неверный формат)
- (ERROR_TEMP_LOST) - два и более раз потерян температурный датчик
- (ERROR_TEMP_TOO_LOW) - слишком  низкая температура
- (ERROR_PIC_LOST) - ошибка инициализации PIC на любой из плат
- ERROR_TEMP_LOST_1ST - перезапустите cgminer, чтобы снова проверить тип датчика температуры (скорее всего, датчик потерян в первый раз)
- ERROR_UNBALANCE - произошел дисбаланс, перезагрузите
- ERROR_TEMP_TOO_HIGH - сверх максимальной температуры
- WARN_NET_LOST - потеря сетевого подключения
- ERROR_NET_LOST - потеря сетевого подключения
